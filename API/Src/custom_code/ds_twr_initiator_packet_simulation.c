...
#if defined(DS_TWR_INITIATOR_PACKET_SIMULATION)
 ...
 * @return none
 */
int ds_twr_initiator_packet_simulation(void)
{

    /* Display application name on LCD. */
    test_run_info((unsigned char *)APP_NAME);

    Init_DelayTimes result = ds_twr_getDelayTime_initiator(&config);

    uint32_t POLL_TX_TO_RESP_RX_DLY_UUS = result.res_rx_final_tx;
    uint32_t RESP_RX_TO_FINAL_TX_DLY_UUS = result.poll_tx_res_rx;
    uint32_t RESP_RX_TIMEOUT_UUS = result.resp_timeout;
