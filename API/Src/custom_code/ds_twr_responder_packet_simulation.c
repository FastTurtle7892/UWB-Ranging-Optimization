...
#if defined(DS_TWR_RESPONDER_PACKET_SIMULATION)
...
static dwt_config_t config = {
    ...
    (33 + 8 - 4),   /* SFD timeout (preamble length + 1 + SFD length - PAC size). Used in RX only. */
...
int ds_twr_responder_packet_simulation(void)
{

    /* Display application name on LCD. */
    test_run_info((unsigned char *)APP_NAME);

    Resp_DelayTimes result = ds_twr_getDelayTime_responder(&config);

    uint32_t POLL_RX_TO_RESP_TX_DLY_UUS = result.poll_rx_res_tx;
    uint32_t RESP_TX_TO_FINAL_RX_DLY_UUS = result.res_tx_final_rx;
    uint32_t FINAL_RX_TIMEOUT_UUS = result.final_timeout;
